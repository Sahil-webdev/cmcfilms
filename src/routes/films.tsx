...
export function WeddingFilmsPage() {
  ...
  return (
    ...
    >
      ...
      {/* ── SECTION 4 — ALL FILMS & TYPOGRAPHIC INDEX LIST (#F2EFE8 Warm Ivory) ── */}
      <section className="py-24 md:py-36 px-6 md:px-16 border-b border-black/10 max-w-[1600px] mx-auto space-y-12">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-black/10 pb-6">
          <div className="space-y-1">
            ...
            <h2 className="font-display text-4xl sm:text-5xl font-light text-[#171512]">
              All <em className="font-editorial italic text-[#A67B2E]">Films</em>
            </h2>
          </div>

          {/* Simple Typography Category Filter */}
              ...
              return (
                <button
                  ...
                  className={`pb-1 transition-all cursor-pointer ${
                    isSelected
                      ? "text-[#171512] border-b-2 border-[#A67B2E] font-bold"
                      : "text-[#171512]/50 hover:text-[#171512]"
                  }`}
                >
                  {cat}
                </button>
        ...
        {/* Floating Cursor Cover Photo Preview */}
        {hoveredIndex !== null && (
          <div
            className="fixed pointer-events-none z-50 w-56 aspect-[16/10] rounded-[2px] overflow-hidden shadow-2xl border border-black/10 transition-opacity duration-300 hidden md:block"
            style={{
          ...
          >
            <img
              src={filteredIndexFilms[hoveredIndex]?.coverImage}
              alt="Film preview"
        ...
        <div className="divide-y divide-black/10 border-t border-b border-black/10">
          {filteredIndexFilms.map((film, idx) => (
            <div
              key={film.id}
              onClick={() => setActiveFilmModal(film)}
              onMouseEnter={() => setHoveredIndex(idx)}
              onMouseLeave={() => setHoveredIndex(null)}
              className="group py-6 flex items-center justify-between gap-4 hover:bg-white/80 transition-colors cursor-pointer px-2"
            >
              <div className="flex items-center gap-6 sm:gap-12">
                <span className="text-xs font-mono text-[#A67B2E] w-8 shrink-0">
                  {film.code}
                </span>

                <h3 className="font-display text-2xl sm:text-4xl font-light text-[#171512] group-hover:text-[#A67B2E] transition-colors">
                  {film.couple.replace("&", "+")}
                </h3>
              </div>

              <div className="flex items-center gap-8 text-xs font-mono text-[#171512]/60">
                <span className="hidden sm:inline">{film.location}</span>
                <span>{film.year}</span>
                <span className="text-[#171512] group-hover:text-[#A67B2E] font-semibold inline-flex items-center gap-1 group-hover:translate-x-1 transition-transform">
                  <span>WATCH</span>
