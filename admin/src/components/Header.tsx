...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="h-20 sticky top-0 z-20 px-6 sm:px-8 flex items-center justify-between transition-colors duration-200 border-b backdrop-blur-xl bg-white/80 dark:bg-[#0B0D14]/80 border-slate-200/80 dark:border-[#1E2235]/80 font-sans">
      {/* Title & Breadcrumb */}
      <div>
        <div className="flex items-center gap-2 text-[11px] font-bold text-[#8C90C1] tracking-wider uppercase">
          <span>CMC Films Studio</span>
          <span>/</span>
          <span className="capitalize">{activeTab}</span>
        </div>
        <h2 className="text-xl sm:text-2xl font-bold tracking-tight text-slate-900 dark:text-white mt-0.5 font-sans">
          {titleInfo.main}
        </h2>
      </div>
      ...
      <div className="flex items-center gap-3 sm:gap-4">
        ...
        <button
          ...
          className="flex items-center gap-3 px-3.5 py-2 rounded-xl border text-xs font-medium transition-all bg-slate-100 hover:bg-slate-200 dark:bg-[#121522] dark:hover:bg-[#1A1E2E] border-slate-200 dark:border-[#202435] text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white cursor-pointer"
        >
        ...
        <button
          ...
          className="flex items-center gap-2 bg-[#8C90C1] hover:bg-[#787CAE] text-white text-xs font-semibold px-4 py-2.5 rounded-xl shadow-lg shadow-[#8C90C1]/20 transition-all duration-200 active:scale-95 cursor-pointer font-sans"
        >
