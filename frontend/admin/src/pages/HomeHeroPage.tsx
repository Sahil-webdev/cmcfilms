...
const heroPages = [
  { key: 'home', label: 'Home', type: 'image', recommendation: '1920 × 1080 px · 16:9 · 3 images' },
...
type HeroMedia = Record<string, { url: string; type: 'image' | 'video'; images?: string[] }>;
...
export const HomeHeroPage: React.FC = () => {
  ...
  const uploadMedia = async (key: HeroKey, file?: File, slot?: number) => {
    if (!file) return;
    ...
    try {
      ...
      if (typeof slot === 'number') formData.append('slot', String(slot));
  ...
  return <div className="mx-auto max-w-6xl space-y-8 p-6 font-sans sm:p-8">
    ...
    <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{heroPages.map((page) => { const saved = media[page.key]; const Icon = page.type === 'video' ? FileVideo : FileImage; const isImageCarousel = page.key === 'home' || page.key === 'couples'; const carouselImages = saved?.images || (saved?.url && saved.type !== 'video' ? [saved.url] : []); return <section key={page.key} className="overflow-hidden rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-[#1E2235] dark:bg-[#121522]"><div className="mb-3 flex items-start justify-between gap-2"><div className="flex items-center gap-2 text-sm font-bold text-slate-900 dark:text-white"><Icon className="h-4 w-4 text-[#8C90C1]" />{page.label}</div><div className="text-right"><span className="block text-[10px] font-medium uppercase text-slate-400">{isImageCarousel ? '3-image carousel' : page.type}</span><span className="mt-0.5 block text-[10px] font-semibold text-[#8C90C1]">Recommended: {page.recommendation}</span></div></div>{isImageCarousel ? <div className="grid grid-cols-3 gap-2">{[0, 1, 2].map((slot) => <label key={slot} className="group relative aspect-[4/5] cursor-pointer overflow-hidden rounded-lg bg-slate-100 dark:bg-[#1A1E2E]">{carouselImages[slot] ? <img src={carouselImages[slot]} alt={`${page.label} slide ${slot + 1}`} className="h-full w-full object-cover" /> : <span className="flex h-full items-center justify-center p-2 text-center text-[10px] text-slate-400">Slide {slot + 1}</span>}<span className="absolute inset-x-1 bottom-1 rounded bg-black/65 py-1 text-center text-[9px] font-semibold text-white"><Upload className="mr-0.5 inline h-2.5 w-2.5" />{carouselImages[slot] ? 'Replace' : 'Upload'}</span><input type="file" accept="image/*" disabled={uploading !== null} className="sr-only" onChange={(event) => uploadMedia(page.key, event.target.files?.[0], slot)} /></label>)}</div> : <><div className="mb-4 aspect-video overflow-hidden rounded-xl bg-slate-100 dark:bg-[#1A1E2E]">{saved?.url ? saved.type === 'video' ? <video src={saved.url} muted loop autoPlay playsInline className="h-full w-full object-cover" /> : <img src={saved.url} alt={`${page.label} hero`} className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center text-center text-[11px] text-slate-400">Website default media is active</div>}</div><label className="flex cursor-pointer items-center justify-center gap-2 rounded-xl bg-[#8C90C1] px-3 py-2.5 text-xs font-semibold text-white transition-colors hover:bg-[#787CAE]"><Upload className="h-4 w-4" />{uploading === page.key ? 'Uploading…' : `Choose ${page.type === 'video' ? 'MP4 video' : 'image'}`}<input type="file" accept={page.type === 'video' ? 'video/mp4,.mp4' : 'image/*'} disabled={uploading !== null} className="sr-only" onChange={(event) => uploadMedia(page.key, event.target.files?.[0])} /></label></>}</section>; })}</div>
  </div>;
};
